Для переключения языков на фронте можно использовать встроенный сниппет **getLocales** (доступен с 1.0.8). Ниже — вариант с кастомным сниппетом `getLanguages` и своей разметкой.
